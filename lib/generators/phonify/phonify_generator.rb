class PhonifyGenerator < Rails::Generators::Base
  ...
  class_option :campaign_name, :default => "Campaign1", :description => "Name your phonify.io campaign"
  class_option :campaign_key, :default => "CHANGEME", :description => "Set your campaign key from phonify.io"
  ...
  def generate_phonify
    ...
    inject_into_file "app/models/#{user.underscore}.rb", :after => "class #{user.camelize} < ActiveRecord::Base\n" do
      "  has_one :#{sanitized_campaign_name}_subscription, #{hash_string(:as => '"owner"', :class_name => '"Phonify::Subscription"', :conditions => '{ ' + hash_string(:campaign_id => "Phonify.config.#{sanitized_campaign_name}") + ' }')}\n"
    end
    initializer("phonify.rb") do
      <<-RUBY.strip_heredoc
      # Campaign settings
      Phonify.config.#{sanitized_campaign_name} = '#{options.campaign_key}'
