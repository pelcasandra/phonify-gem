class Phonify::Phone < ActiveRecord::Base
  ...
  QUERY_ATTRS = [:app_id, :number, :country, :carrier, :offset, :limit]
  include Phonify::Base

  def messages(scope = Phonify::Message)
    return Phonify::Message.where(:phone_id => self.id) if scope == :local
    Phonify::Message::Collection.new(scope.where(:phone_id => self.id), :destination => { :id => self.token }, :app_id => self.app_id)
  end
